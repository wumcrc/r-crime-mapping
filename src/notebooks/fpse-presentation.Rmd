...
dst_2_pop <- as.data.frame(dst_2_pop)
summary_statistics <-
  list(
    "Crime Rates" =
      list(
...
dst_2_table <- summary_table(dst_2_pop, summary_statistics)
...
```{r Larceny Kables, include = FALSE}
larceny %>% 
  filter(., neighborhood == 39) %>%
  group_by(weekday) %>% 
  count() %>%
