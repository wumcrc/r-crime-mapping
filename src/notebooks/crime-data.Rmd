...
augustCrimes18 <- cs_extract_month(yearList18, month = "August")
```

### Clean & Categorize Data - 2018

```{r Filter & Categorize 2018 Data}
tidyAugustCrimes18 <- augustCrimes18 %>% 
  cs_filter_count(., var = count) %>%
  cs_filter_crime(., var = crime, "part 1") %>%
  cs_crime_cat(., var = crime, crimeCatNum, "numeric") %>%
  cs_crime_cat(., var = crime, crimeCatName, "string") %>%
...
tidyTotalCrimes18$tod <- strptime(tidyCrimes18$tod, tz = "America/Chicago", "%H:%M")
tidyTotalCrimes18$tod <- format(tidyCrimes18$tod, format = "%H%M%S")
...
augustCrimes <- rbind(tidyTotalCrimes18, tidyCrimes19)
```

### Create Spatial Objects

```{r Create SF objects for 2018 & 2019 Crimes}
crimes18_sf <- cs_projectXY(tidyTotalCrimes18, varX = x_coord, varY = y_coord, crs = 102696)
