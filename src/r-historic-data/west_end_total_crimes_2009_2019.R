...
we_crimes_tidy$monthVar <- month(as.Date(we_crimes_tidy$monthVar, 
                                      format="%d/%m/%Y"), label = TRUE)

we_crimes_tidy$tod <- strptime(we_crimes_tidy$tod, 
                            tz = "America/Chicago", "%H:%M")

we_crimes_tidy$tod <- format(we_crimes_tidy$tod, 
                          format = "%H%M%S")

we_crimes_tidy <- we_crimes_tidy %>%
...
we_crimes_tidy <- mutate(we_crimes_tidy, "type" = ifelse(grepl("ALL OTHER", description), "All Other", 
...
we_crimes_tidy <- mutate(we_crimes_tidy, "value" = ifelse(grepl("UNDER", description), "Under $500",
                                            ifelse(grepl("\\$500 - \\$24,999", description), "$500 - $24,999",
                                                   ifelse(grepl("OVER \\$25,000", description), "Over $25,000", NA))))

write_xlsx(we_crimes_tidy, here("results", "historic-data", "west_end_total_crimes_2009_2019.xlsx"))