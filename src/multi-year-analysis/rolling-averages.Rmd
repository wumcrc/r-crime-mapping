...
```{r}
fpse_crimes %>% 
  group_by(crimeCatName, yearVar) %>%
  filter(., yearVar <= 2018 ) %>%
  count() -> fpse_cat_table

fpse_cat_df <- as.data.frame(fpse_cat_table)

fpse_cat_df <- fpse_cat_df %>% 
  filter(., crimeCatName == "Aggravated Assault") %>%
  mutate(n01 = rollmean(n, k = 3, fill = NA, align = "right"))
...
fpse_cat_df %>%
  ...
  geom_point() + 
  labs(title = "Forest Park Southeast",
       subtitle = "Aggravated Assault - 3 Year Rolling Crime Averages",
       x = "Year", 
       y = "Number of Crimes") +
  scale_color_discrete(name="",
                       breaks = c("n", "n01"),
