...
```{r}
augustTidy$tod <- strptime(augustTidy$tod, tz = "America/Chicago", "%H:%M")
augustTidy$tod <- format(augustTidy$tod, format = "%H%M%S")

augustTidy <- augustTidy %>%
...
```{r}
augustTidy <- cs_missingXY(augustTidy, varX = x_coord, varY = y_coord, newVar = missing)
table(augustTidy$missing)
